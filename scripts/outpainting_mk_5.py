...
class Script(scripts.Script):
    ...
    def run(self, p, _, pixels, mask_blur, direction, noise_q, color_variation):
        print("$$$ - Rekil - outpainting_mk 5 function")
        ...
        left = pixels if "left" in direction else 0 #128
        right = pixels if "left" in direction else 0
        up = pixels if "up" in direction else 0 #0
        down = pixels if "up" in direction else 0 #0
        ...
        if right > 0: 
            right = target_w - init_img.width - left  #140 
        ...
        def expand(init, count, expand_pixels, is_left=False, is_right=False, is_top=False, is_bottom=False):
            ...
            pixels_horiz = 2*expand_pixels if is_horiz else 0
            pixels_vert  = 2*expand_pixels if is_vert else 0
            ...
            for n in range(count):
                ...
                img.paste(init[n], (expand_pixels if is_left else 0, expand_pixels if is_top else 0))
